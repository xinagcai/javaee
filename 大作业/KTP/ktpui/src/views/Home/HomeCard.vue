  ...
  <div class="card">
    ...
    <div class="homework">
      ...
      <ul style="padding-left: 0px;margin-top: 5px">
        <li v-for="(item,index) in works" style="margin-bottom: 3px"><span v-if="index<3">{{item.tname}}</span></li>
      </ul>
...
export default {
  ...
  mounted() {
    ...
    // 请求成员数量
    request({
      url: '/api/course/getCourseMembers',
      method: 'get',
      params: {
        cno: this.course.cno
      }
    }).then(res => {
      this.memberNum = res.data.length
      this.send()
    }).catch(err => {
      console.log(err)
    })
