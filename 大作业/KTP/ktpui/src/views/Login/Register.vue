  ...
  <div class="outer">
    <div>
      <div v-if="status === 1" class="register-home">
        ...
        <div>
          <div><img src="../../assets/img/teacher.png" style="display: inline-block; float: left">
            <div @click="status = 3" style="display: inline-block;text-align: center;background: #32baf0">我是老师/助教</div>
          </div>
          <div><img src="../../assets/img/student.png" style="display: inline-block; float: left">
            <div @click="status = 2" style="display: inline-block;text-align: center;background: #32baf0">我是学生</div>
          </div>
      ...
      <div v-else-if="status === 2" class="register-student">
        <div>
          <div><i class="el-icon-back" @click="changeStatus"/></div>
          <div>学生注册</div>
        </div>
        <div>
          <div><input type="text" placeholder="账号" v-model="account" maxlength="20" @blur="verifyAccountExist"
                      @focus="focusBorder(1)"/>
            <div></div>
          </div>
          <div><input type="password" placeholder="密码" v-model="pwd" maxlength="16" @focus="focusBorder(2)"
                      @blur="notNullContent(2, $event)"/>
            <div>密码为必填项目</div>
          </div>
          <div><input type="password" placeholder="再次输入密码" maxlength="16" v-model="pwdAgain" @blur="verifyPwd"
                      @focus="focusBorder(3)"/>
            <div></div>
          </div>
          <div><input type="text" placeholder="姓名" v-model="name" @focus="focusBorder(4)"
                      @blur="notNullContent(4, $event)"/>
            <div>姓名为必填项目</div>
          </div>
          <div><input type="text" placeholder="学号" v-model="identityNo" @focus="focusBorder(5)"
                      @blur="notNullContent(5, $event)"/>
            <div>学号为必填项目</div>
          </div>
          <div>
            <el-select
        ...
        <div>
          <div class="for-student" @click="register">注册为学生</div>
        </div>
        <div class="login"><span @click="login">已有账号？</span><span @click="login">去登录</span></div>
      </div>
      <div v-else class="register-teacher">
        ...
        <div>
          <div><input type="text" placeholder="账号" maxlength="20" v-model="account" @blur="verifyAccountExist"
                      @focus="focusBorder(1)"/>
            <div></div>
          </div>
          <div><input type="password" placeholder="密码" v-model="pwd" maxlength="16" @focus="focusBorder(2)"
        ...
        <div>
          <div class="for-teacher" @click="register">注册为老师/助教</div>
        </div>
        <div class="login"><span @click="login">已有账号？</span><span @click="login">去登录</span></div>
      </div>
    </div>
...
<script>
  ...
  import {IS_LOGIN, USER_AVATAR, CHANGE_UID, PERMISSION, TOKEN} from "../../store/mutation-type"
            ...
            if (this.identityNo !== '') {
              request({
                ...
                data: {
                  ...
                  school: this.school,
                  identifyNo: this.identityNo.slice(0, 10),
...
<style scoped>
  .outer {
    ...
    background: url("../../assets/img/reg-bg-01.jpg") center center no-repeat;
  ...
  .register-home > div:nth-child(3) > div > div {
    height: 64px;
    width: 320px;
    border-radius: 10px;
    cursor: pointer;
