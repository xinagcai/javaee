<template>
  <div class="outer">
    <div>
      <div>账号登录</div>
      <div>
        <div><input type="text" maxlength="20" placeholder="请输入账号" v-model="account" /></div>
        <div><input type="password" maxlength="16" placeholder="请输入密码" v-model="pwd" /></div>
        <div><el-checkbox v-model="checked">下次自动登录</el-checkbox></div>
        <div><div class="login-btn" @click="toLogin">登录</div></div>
...
import {IS_LOGIN, CHANGE_UID, USER_AVATAR, PERMISSION, CHANGE_NAME, TOKEN} from "../../store/mutation-type"
...
<style scoped>
  .outer {
    ...
    align-items:center;
    background:url("../../assets/img/login-bg2-01.jpg");
